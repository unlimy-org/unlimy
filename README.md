...
- В главном меню есть раздел `Поддержка` с тикет-системой (создание обращения, история тикетов, ответы).
...
- открыть `Поддержка` и создать тестовый тикет.

## Тикет-система поддержки
- Пользователь: `Поддержка` -> `Создать тикет` -> отправляет один текстовый месседж.
- Бот создает тикет и отправляет уведомление администраторам из `SUPPORT_ADMIN_IDS`.
- Администратор: `Поддержка` -> `Открытые тикеты (админ)` -> открывает тикет -> `Ответить`.
- Пользователь видит ответы в том же треде тикета (`Поддержка` -> `Мои тикеты`).
